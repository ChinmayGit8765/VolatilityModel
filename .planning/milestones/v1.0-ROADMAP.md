# Milestone v1.0: VolForecast — Crypto + Stock Volatility Forecasting MLOps Platform
...
Empty repo to closed-loop volatility forecasting platform: validated cross-asset ingestion (ccxt + yfinance, Pandera, DVC), leak-free purged walk-forward evaluation of EWMA/GARCH(1,1)/HAR-RV baselines, an MLflow-tracked LightGBM challenger benchmarked honestly on identical folds (it loses — stated plainly), Dockerized FastAPI serving with an append-only prediction log, a genuinely closed feedback loop (auto-arriving labels, Evidently drift, rolling-QLIKE performance monitoring, QLIKE-gated champion/challenger promotion, Prefect daily flow), and a Streamlit dashboard + honest MODEL_CARD.

## Milestone Summary

**Stats:** 5 phases | 20 plans | 200 commits | 2026-06-10 → 2026-07-19 | ~20k LOC Python | 485 tests (37 test files) | CI green on GitHub

**Key accomplishments:**
1. Leak-free evaluation as the credibility centerpiece — purged/embargoed walk-forward harness with value-level alignment tests; two independent leakage defects (baseline one-day misalignment; ML train-on-test contamination) caught by review and fixed, with the published bar regenerated each time.
2. Honest headline result: leak-free LightGBM does NOT beat HAR-RV/EWMA on QLIKE at daily horizon on any asset — documented per asset and per regime in reports/ml_vs_baselines.md and MODEL_CARD.md.
3. Full MLOps lifecycle live on docker-compose: Postgres-backed MLflow (alias registry, --serve-artifacts), Prefect daily flow (ingest → features → forecast → label → drift → performance → conditional retrain/promotion), FastAPI champion serving, Streamlit dashboard.
4. Closed feedback loop verified live end-to-end: the daily flow generates forecasts, labels arrive automatically from ingested closes, drift + performance monitors run on real data, promotion is QLIKE-gated with no-promote default and alias-flip rollback.
5. Milestone integration audit caught two cross-phase blockers invisible to unit tests (inert feedback loop; drift parser targeting a schema Evidently never produces) — both fixed and re-verified live before completion.
6. CI-hermetic test discipline throughout: 485 offline tests, fixture-based, green on GitHub Actions on every sealed phase.

**Issues Resolved:** silent no-op ingest in the daily flow (missing __main__ guard); eval column-order transpose; stale drift reference; Windows-specific traps (cp1252 console, CRLF, SQLite-on-bind-mount avoided by design).

**Issues Deferred:** alerts.jsonl has no dashboard consumer; /forecast latency ~40-60s (synchronous GARCH-as-feature); Info-tier review findings (see phase REVIEW.md fix-status tables); cloud deploy, LSTM challenger, intraday RV, Feast (v2 stretch).
